...
try:
    conf = import_module(mod_name)
    defPdf = conf.defPdf
except Exception as e:
    print(f"定義ファイル読み込みでエラーが発生しました\n{e}\n処理を中止します。")
    sys.exit()


if hkey == '':
    hkey = defPdf['default']['param']['key']


#----　DB Read ---
#db = defPdf['DB']['database']

#conn = sqlite3.connect(db)

htable = defPdf['DB']['table_head']
btable = defPdf['DB']['table_body']


link_key = defPdf['DB']['link_key']
...
#nouHs = dict_select_all_key(conn, htable,{link_key:hkey }, link_key)
#nouBs = dict_select_all_key(conn, btable,{link_key:hkey }, link_key)

nouHs = d_data['data']['hdata']
nouBs = d_data['data']['bdata']

_HEAD = nouHs[0]
_ROWS = nouBs

#dict_data = {'data': {'hdata': nouHs ,'bdata': nouBs}}
...
doc = BaseDocTemplate(pfile, pagesize=A4)
#doc = BaseDocTemplate(sys.stdout, pagesize=A4)



#top_mergin = 20*mm
top_mergin = defPdf['attr']['top_mergin']
ft_size = defPdf['attr']['footter_size']
...
#-----タイトル表示 ------
elements = []
elements.append(defPdf['header']['title'])
